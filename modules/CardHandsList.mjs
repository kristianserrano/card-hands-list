    ...
    async getData(options = {}) {
        ...
        for (const hand of hands) {
            if (hand.hasPlayerOwner) {
                ...
                for (const key of Object.keys(hand.ownership)) {
                    if (key !== 'default' && key !== game.users.activeGM.id && hand.ownership[key] === CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER) {
                        owners.push(game.users.get(key))
                    }
                }
        ...
        const data = {
            title: this.title,
            hands,
            handsOwned: hands.filter(h => h.ownership[game.user.id] === CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER).length,
            handsObservable: hands.filter(h => h.ownership[game.user.id] === CONST.DOCUMENT_OWNERSHIP_LEVELS.OBSERVER).length,
